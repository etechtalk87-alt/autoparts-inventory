...
const filePath = path.join(__dirname, 'src', 'pages', 'CreateInvoice.jsx');
let text = fs.readFileSync(filePath, 'utf-8');

let startIdx = text.indexOf('  return (\n    <main className="min-h-screen');
if (startIdx === -1) {
    startIdx = text.indexOf('  return (\n');
}

const modalStart = text.indexOf('{showCustomerModal ? (');
const modalEndStr = '            </div>\n\n            <div className="space-y-4 rounded-[1.75rem]';
const modalEnd = text.indexOf(modalEndStr);
if (modalStart === -1 || modalEnd === -1) {
    console.error("Could not extract modal code.");
    process.exit(1);
}
const modalCode = text.substring(modalStart, modalEnd).trim();
...
fs.writeFileSync(filePath, text.substring(0, startIdx) + newReturn, 'utf-8');
console.log("Successfully updated CreateInvoice.jsx");
